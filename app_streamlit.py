...
selected_teams = st.sidebar.multiselect("Team", options=sorted(pred_df["team"].unique()))
min_pp = st.sidebar.slider("Min predicted points", float(pred_df["predicted_points"].min()), float(pred_df["predicted_points"].max()), float(pred_df["predicted_points"].min()))
...
if sel_df.empty:
    ...
    st.dataframe(sel_df[["name","team","position_name","value","predicted_points"]].sort_values("predicted_points", ascending=False), use_container_width=True)
    ...
    def suggest_starting_xi(squad_df):
        ...
        non_gk = squad_df[squad_df["position_name"]!="GK"].sort_values("predicted_points", ascending=False)
        ...
        non_gk_top = non_gk.nlargest(10, "predicted_points")
        starters = pd.concat([gk, non_gk_top])
        return starters.sort_values("predicted_points", ascending=False)
...
pred_df["points_per_value"] = pred_df["predicted_points"] / pred_df["value"]
fig2 = px.scatter(pred_df, x="value", y="predicted_points", color="position_name", hover_data=["name","team"])
